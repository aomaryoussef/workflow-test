...
func (t *MultiInstruction) Begin(ctx context.Context) error {
	...
	if err != nil {
		logging.LogHandle.WithContext(ctx).Errorf("failed to start transaction %v", err)
		return err
	}
	t.tx = tx
	return nil
...
func (t *MultiInstruction) Commit(ctx context.Context) error {
	err := t.tx.Commit()
	if err != nil {
		logging.LogHandle.WithContext(ctx).Errorf("failed to commit transaction %v", err)
	}
	return err
}
...
func (t *MultiInstruction) Rollback(ctx context.Context) error {
	err := t.tx.Rollback()
	if err != nil {
		logging.LogHandle.WithContext(ctx).Errorf("failed to rollback transaction %v", err)
	}
	return err
}
