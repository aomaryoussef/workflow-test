...
from src.domain.registry.use_cases.update_payment_status import UpdatePaymentStatusUseCase

logger = logger.bind(service="registry", context="controller", action="update payment")


class UpdatePaymentController(BaseController):
    def __init__(self, id: str, request: dict):
        ...
        self.input_dto: InputDto = InputDto(id=payment_id, status=request.status)

    def execute(self):
        logger.debug("execute")
        try:
            use_case = UpdatePaymentStatusUseCase(repository=self.repository, input_dto=self.input_dto)
            output_dto: OutputDto = use_case.execute()
            result = RegistryViews.show(output_dto)
