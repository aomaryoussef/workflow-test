...
export class BaseRepository<T> {
  ...
  /**
   * Finds a single entity based on the provided filters.
   *
   * This method retrieves an entity from the database using the provided filters.
   * It utilizes TypeORM's `findOneBy` method to query the entity.
   * Supports optional transaction via `EntityManager`.
   *
   * @param filters - An object containing the filter conditions.
   * @param manager - Optional `EntityManager` for transactional operations.
   * @returns A promise that resolves to the found entity or `null` if not found.
   *
   * @example
   * ```typescript
   * const entity = await baseRepository.findOne({ transaction_id: 123456 });
   * ```
   */
