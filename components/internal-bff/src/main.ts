...
async function bootstrap() {
  ...
  const swaggerDescription = `
    This is the definition of **mylo internal bff REST APIs** v${MAJOR_VERSION}.${MINOR_VERSION}.${PATCH_VERSION}
  ...
  const config = new DocumentBuilder()
    .setTitle('mylo internal bff APIs')
    .setDescription(swaggerDescription)
    .setVersion(`${MAJOR_VERSION}.${MINOR_VERSION}.${PATCH_VERSION}`)
    .build();

  const document = SwaggerModule.createDocument(app, config);
  SwaggerModule.setup('internal-bff/api/docs', app, document); // Swagger will be available at /api
