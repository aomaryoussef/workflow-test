...
var _ job.JobArgsWithHeader = (*GlLoanCancellation)(nil)

type GlLoanCancellation struct {
...
func (a GlLoanCancellation) MessageHeader() job.MessageHeader {
	return a.Header
}

func (a GlLoanCancellation) Kind() string {
	return types.JobTypeGLPostLoanCancellation.String()
}

func (gl *GeneralLedgerERPJobHandler) PostLoanCancellation(ctx context.Context, args job.JobArgsWithHeader) error {
	...
	request := args.(GlLoanCancellation)
	...
	postings, err := entriesLoanCancellation(request)
	if err != nil {
	...
	glReconciliationWm := storage.GlReconciliation{
		...
		LinkedEventType:  pbcommon.EventType_LOAN_ACCOUNT_CANCELED.String(),
...
func entriesLoanCancellation(la GlLoanCancellation) (*dynamicsgen.PostJournalEntries, error) {
			...
			GeneralJournalList: []dynamicsgen.GeneralJournal{
				{
					...
					JournalName:               fmt.Sprintf("%s%s", JournalPrefixReverseLoanActivation, extractLast2DigitsFromYear(la.BookedAt)),
					...
					JournalLines: []dynamicsgen.JournalLine{
						newJournalLine(dynamicsgen.Credit, la.PrincipalUnits, common.MurabahaPurchaseAccount.GeneralLedgerAccount, "Murabaha Purchase to Merchant"),
						newJournalLine(dynamicsgen.Debit, la.PrincipalUnits, common.MurabahaPrincipalReceivableAccount.GeneralLedgerAccount, "Murabaha Purchase from mylo Treasury"),
						newJournalLine(dynamicsgen.Credit, la.TotalInterestReceivableUnits, common.MurabahaInterestReceivableAccount.GeneralLedgerAccount, "Total interest receivable from mylo Treasury"),
						newJournalLine(dynamicsgen.Debit, la.TotalInterestReceivableUnits, common.MurabahaUnearnedRevenueAccount.GeneralLedgerAccount, "Total interest unearned revenue to mylo Treasury"),
						newJournalLine(dynamicsgen.Credit, la.DoubtfulAllowanceUnits, common.DoubtfulReceivableAccount.GeneralLedgerAccount, "Doubtful allowance (bad debt) receivable to mylo Treasury"),
						newJournalLine(dynamicsgen.Debit, la.DoubtfulAllowanceUnits, common.DoubtfulAccount.GeneralLedgerAccount, "Doubtful allowance (bad debt) from mylo Treasury"),
					},
				},
				{
					EnteredBy:                 commonComment,
					HeaderResponseStatus:      EmptyString,
					HeaderResponseDescription: EmptyString,
					JournalName:               fmt.Sprintf("%s%s", JournalPrefixReverseFees, extractLast2DigitsFromYear(la.BookedAt)),
					MyloJournalReference:      EmptyString,
					MyloTransType:             "Admin Fees",
					TransDate:                 transformToDynamicsDateFmt(time.Now()),
					JournalLines: []dynamicsgen.JournalLine{
						newJournalLine(dynamicsgen.Credit, la.AdminFeeCollectedByMerchantUnits, common.AdminFeeReceivableAccount.GeneralLedgerAccount, "Admin fee collected by merchant to mylo Treasury"),
						newJournalLine(dynamicsgen.Credit, la.VATCollectedByMerchantUnits, common.VatFeeReceivableAccount.GeneralLedgerAccount, "VAT collected by merchant to mylo Treasury"),
						newJournalLine(dynamicsgen.Debit, la.AdminFeeCollectedByMerchantUnits+la.VATCollectedByMerchantUnits, fmt.Sprintf("%s|%s|MISC", common.MerchantDueAccount.GeneralLedgerAccount, la.MerchantCode), "Admin fee and VAT collected by merchant from mylo Treasury"),
					},
				},
				{
					EnteredBy:                 commonComment,
					HeaderResponseStatus:      EmptyString,
					HeaderResponseDescription: EmptyString,
					JournalName:               fmt.Sprintf("%s%s", JournalPrefixReversePurchase, extractLast2DigitsFromYear(la.BookedAt)),
					MyloJournalReference:      EmptyString,
					MyloTransType:             "Purchase",
					TransDate:                 transformToDynamicsDateFmt(time.Now()),
					JournalLines: []dynamicsgen.JournalLine{
						newJournalLine(dynamicsgen.Credit, la.PrincipalUnits, fmt.Sprintf("%s|%s|MISC", common.MerchantDueAccount.GeneralLedgerAccount, la.MerchantCode), "Pay to Merchant"),
						newJournalLine(dynamicsgen.Debit, la.PrincipalUnits, common.MurabahaPurchaseAccount.GeneralLedgerAccount, "Purchase from Merchant"),
					},
				},
			},
		},
