...
func (c *Commands) CancelLoanAccount(ctx context.Context, cmd *types.CancelLoanAccountCmd) (*types.CmdProcessorResult, error) {
	log := logging.WithContext(ctx)
	ctx, span := otelx.NewSpanFromContext(ctx, "commands#CancelLoanAccount")
	defer span.End()
	...
	if c.allowLoanCancellationAfterInterestRecognition {
		loanAccountAggregate.SetAllowLoanCancellationAfterInterestRecognition()
	}

	event, err := loanAccountAggregate.ApplyCommand(cmd)
	if err != nil {
		return nil, err
	}
	...
	err = c.publishEvent(ctx, func() river.JobArgs {
		return loanaccountjob.CancelLoanAccount{
	...
	err = c.publishEvent(ctx, func() river.JobArgs {
		return sljob.SlLoanCancellation{
			Header:                           job.MessageHeaderFromContext(ctx),
			EventId:                          event.Id,
			LoanId:                           event.AggregateId,
	...
	err = c.publishEvent(ctx, func() river.JobArgs {
		return gljob.GlLoanCancellation{
			Header:                           job.MessageHeaderFromContext(ctx),
			EventId:                          event.Id,
			LoanId:                           event.AggregateId,
			LenderSource:                     loanAccountAggregate.LenderSource,
