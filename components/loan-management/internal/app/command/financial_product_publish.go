...
func (c *Commands) PublishFinancialProduct(ctx context.Context, cmd *types.PublishFinancialProductCmd) (*types.CmdProcessorResult, error) {
	log := logging.WithContext(ctx)
	ctx, span := otelx.NewSpanFromContext(ctx, "commands#PublishFinancialProduct")
	defer span.End()
	...
	err = c.publishEvent(ctx, func() river.JobArgs {
		return financialproductjob.UpdateFinancialProductState{
			...
			EventId:            event.Id,
			EventType:          pbcommon.EventType_FINANCIAL_PRODUCT_PUBLISHED.String(),
			BookedAt:           event.CreatedAt.ToStdLibTime(),
			UpdatedBy:          event.CreatedBy,
			FinancialProductId: event.AggregateId,
			Version:            event.AggregateVersion.Primitive(),
			NewState:           pbevent.FinancialProductState_PUBLISHED.String(),
		}
