...
import {
  PostMerchantInvoicesInputDTO,
  PostMerchantInvoicesOutputDTO
} from "../../../domain/merchant/dto/tasks/post-merchant-invoices.dto";

@Injectable()
export class PostMerchantInvoicesWorker implements ConductorWorker {
  taskDefName = 'post_merchant_invoices_task';
  pollInterval = 5000;
  ...
  constructor(
    ...
    this.logger = this.loggerFactory(PostMerchantInvoicesWorker.name);
  }
  ...
  async execute(task: Task): Promise<any> {
    ...
    try {
      ...
      const outputData = await this.merchantInvoiceUseCase.postMerchantInvoices(inputDto);
      this.logger.debug(
        `\`${this.taskDefName}\` task completed successfully: ${JSON.stringify(outputData)}`,
      );
      return {
        outputData: new PostMerchantInvoicesOutputDTO({connectorsResponses : outputData}),
